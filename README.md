...
- Ilha de 6.656 × 5.376 pixels organizada em Centro Velho, Bairro da Estação, Jardins do Leste e Zona Industrial. São 46 casas, três construções isométricas, 21 corredores viários, 2.184 células de mapa, 72 árvores e oito carros. As fachadas ficam recuadas das vias; as ruas formam quarteirões, acessos e cruzamentos contínuos. O lago e a costa usam terreno próprio, sem asfalto sob a água. Água reduz a velocidade e gasta energia; cruzar a faixa profunda externa causa afogamento.
...
- O prólogo ocupa a tela inteira com fundo preto e a marca do jogo. O botão libera o menu imediatamente, mesmo se os recursos do Phaser ainda estiverem carregando, e aproveita o gesto para desbloquear o áudio quando disponível.
...
`npm test`: testes automatizados de regras, grade, peso, durabilidade, consumo por instância, drops, validação de saves, comandos, crafting e limites de histórico. `npm run build`: TypeScript, build Vite e cópia somente dos assets usados em runtime. A validação visual cobre prólogo, menu, lobby História, sobrevivência, HUD, malha urbana e zona nuclear em navegador real.
...
- 46 casas e 21 corredores viários formam a malha conectada. O asfalto usa os tiles originais de `assets/Map/Roads`, convertidos sem alterar seus pixels de TGA para PNG por `npm run roads`. Tiles que intersectam o oceano ou o lago são excluídos. O chão usa as texturas de `assets/Map/Grass`, sem asfalto embutido. Decoração de rua em `Objects`; oito carros originais preservados.
